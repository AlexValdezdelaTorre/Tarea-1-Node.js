...
class AuthMiddleware {
    static protec(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const paylod = (yield jwt_adapter_1.JwtAdapter.validateToken(token));
                if (!paylod)
                    return res.status(401).json({ message: "Invalid token" });
                const user = yield data_1.Users.findOne({
                    where: {
                        id: paylod.id,
                        status: data_1.Status.AVAILABLE,
                    },
                });
                if (!user)
...
AuthMiddleware.restricTo = (...roles) => {
    return (req, res, next) => {
